GENERIC BUILD NOTES
====================
Some notes on how to build Stealeum Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build Stealeum with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Stealeum Core requires some essential build tools to be installed before. Please see
...
---------------------
Stealeum inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Stealeum. These
dependencies must be built before Stealeum can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build Stealeum Core. Caching of build results will ensure that only
...
not done, build failures might occur when building Stealeum.
...
ccache
------
`./configure` of Stealeum Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
