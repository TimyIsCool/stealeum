...
# stealeum_hash
RUN git clone https://github.com/stealeum/stealeum_hash
RUN cd stealeum_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} stealeum
RUN useradd -u ${USER_ID} -g stealeum -s /bin/bash -m -d /stealeum stealeum
...
RUN mkdir /stealeum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /stealeum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /stealeum-src
