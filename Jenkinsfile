...
// which are part of the Stealeum repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="stealeum-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /stealeum-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "stealeum-stealeum/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from stealeum-stealeum/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'stealeum-stealeum/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from stealeum-stealeum/develop"
            }
          }

          if (hasCache) {
            sh "cd /stealeum-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /stealeum-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /stealeum-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /stealeum-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /stealeum-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /stealeum-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /stealeum-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
