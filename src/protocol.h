...
enum ServiceFlags : uint64_t {
    ...
    // set by all Stealeum Core nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Stealeum Core does not support this but a patch set called Bitcoin XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Stealeum Core nodes used to support this by default, without advertising this bit,
...
enum GetDataMsg {
    ...
    MSG_SPORK = 6,
    /* 7 - 15 were used in old Stealeum versions and were mainly budget and MN broadcast/ping related*/
    MSG_DSTX = 16,
