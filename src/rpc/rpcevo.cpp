...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The stealeum address to send the collateral to.\n"
        },
		{"collateralAmount",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The stealeum address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The stealeum address to use for smartnode reward payments.\n"
        },
...
// Allows to specify Stealeum address or priv key. In case of Stealeum address, the priv key is taken from the wallet
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Stealeum\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stealeum address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stealeum address: ") + request.params[5].get_str());
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stealeum address: ") + request.params[4].get_str());
...
void protx_quick_setup_help(CWallet* const pwallet)
{
    throw std::runtime_error(
			...
			"  \"stealeum.conf\" :       (string) The content of stealeum.conf to be used in vps node.\n"
            "}\n"
			"\nExamples:\n"
...
UniValue createConfigFile(string blsPrivateKey, string ip, string address) {
	...
	string fileName = get_current_dir() + "/" + address + "_stealeum.conf";
...
//register_prepare "collateralHash" collateralIndex "ipAndPort" "ownerAddress" "operatorPubKey" "votingAddress" operatorReward "payoutAddress" ( "feeSourceAddress" )
	...
	result.push_back(Pair("stealeum.conf",config.get_str()));
