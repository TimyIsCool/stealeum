...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on stealeumcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in stealeum.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("stealeum.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in stealeum.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Stealeum features */
    { "stealeum",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "stealeum",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "stealeum",               "gobject",                &gobject,                true,  {} },
    { "stealeum",               "voteraw",                &voteraw,                true,  {} },
