...
const char * const BITCOIN_CONF_FILENAME = "stealeum.conf";
const char * const BITCOIN_PID_FILENAME = "stealeumd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StealeumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StealeumCore
    // Mac: ~/Library/Application Support/StealeumCore
    // Unix: ~/.stealeumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StealeumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StealeumCore";
#else
    // Unix
    return pathRet / ".stealeumcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty stealeum.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override stealeum.conf
