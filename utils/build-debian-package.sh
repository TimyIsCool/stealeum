...
PACKAGE_VERSION=$($SRC_PATH/src/stealeumd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/stealeumd $DEB_BIN
cp $SRC_PATH/src/stealeum-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/stealeum-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/stealeumd.1 $DEB_MAN
cp $SRC_DOC/man/stealeum-cli.1 $DEB_MAN
cp $SRC_DOC/man/stealeum-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/stealeumd.bash-completion $DEB_CMP/stealeumd
cp $SRC_PATH/contrib/stealeum-cli.bash-completion $DEB_CMP/stealeum-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/stealeumd.1
gzip --best -n $DEB_MAN/stealeum-cli.1
gzip --best -n $DEB_MAN/stealeum-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/stealeumd $DEB_BIN/stealeum-cli
